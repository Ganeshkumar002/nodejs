...
const getBookYear = async function(req,res){
    ...
    let yearData = await bookModel.find(data).select( { year:{ $eq: data}  }).select({bookName:1, _id:0});
...
const getParticularBooks = async function (req,res){
    ...
    let data = req.body
    let yearData = await bookModel.find(data).select({bookName:1 , _id:0});
    res.send({msg:yearData})
...
const getXINRBooks= async function(req,res){
    let allBookData = await bookModel.find({"price" : { $in :["200","250","500"]},}).select({bookName:1,_id:0});
    res.send({ allBookInfo: allBookData});
};
...
const getBooks= async function (req,res) {
    let allBookData = await bookModel.find({totalPages : {$gte :500},stockAvailable : true }).select({bookName: 1,_id: 0});
    res.send({allBookInfo: allBookData});
};
